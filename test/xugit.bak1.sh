...
USB_DEPENDENT="trash-cli fastfetch curl gawk sed grep"                                           # 本脚本依赖的程序
...
while [[ $i -lt "${#USB_ARR[*]}" ]]; do
    ...
    # 由本地与U盘关联的仓库实际生成目录
    USB_LOCAL_MAP_ALL+=($(ls -lid $HOME/"${USB_ARR[$i]##*/}$USB_TAG"/*|awk '{print $9}')) # 获取目录
    USB_LOCAL_ID+=($(ls -lid $HOME/"${USB_ARR[$i]##*/}$USB_TAG"/*/|awk '{print $1}'))      # 获取目录ID
    let i+=1
done
# 获取远程和本地交集，用于同步
...
#
# 使用关联数组存储U盘和HOME分区硬盘的UUID
# declare -A USB_UUID ; i=0
# USB_UUID["HOME"]=$(lsblk -o UUID,MOUNTPOINT |grep "/home"|awk '{print $1}')
# while [[ $i -lt ${#USB_ARR[*]} ]]; do
#     USB_UUID["${USB_ARR[$i]}"]=$(lsblk -o UUID,MOUNTPOINT |grep "${USB_ARR[$i]}"|awk '{print $1}')
#    let i+=1
# done
# i=0 # 关联数组记录本地硬盘上已经克隆的仓库id号
# while [[ $i -lt "${#USB_LOCAL_MAP_ALL[*]}" ]]; do
#     # USB_UUID["${USB_LOCAL_MAP_ALL[$i]}"]=$(ls -lid ${USB_LOCAL_MAP_ALL[$i]}|awk '{print $1}')
#     USB_UUID["${USB_LOCAL_MAP_ALL[$i]}"]="${USB_LOCAL_ID[$i]}"
#     let i+=1
# done
#
# echo ${USB_UUID["${USB_ARR[0]}"]}
# echo ${USB_UUID["HOME"]}
# echo ${USB_UUID["${USB_LOCAL_MAP_ALL[0]}"]}
# echo ${USB_UUID["${USB_LOCAL_MAP_ALL[1]}"]}
# echo ${USB_UUID["${USB_LOCAL_MAP_ALL[2]}"]}
# echo ${USB_UUID["${USB_LOCAL_MAP_ALL[3]}"]}
# echo ${USB_UUID["${USB_LOCAL_MAP_ALL[4]}"]}
#------------2024-06-24-----------------
# echo ${USB_REMORT_MAP_ALL[*]}
# echo ${USB_CHECK_MAP_ALL[*]}
# echo ${USB_LOCAL_MAP_ALL[*]}
# echo ${USB_SYNC_MAP_ALL[*]}
# echo ${USB_RCLONE_MAP_ALL[*]}
# echo ${USB_ARR[*]}
#---------------2024-06-27-------------------------
exit
