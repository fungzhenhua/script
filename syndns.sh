...
SYNDNS_PROCESS(){
    ...
    if [[ $SYN_TES_FIRST != "nameserver 127.0.0.1" ]]; then
        echo "# Generated by syndns" | sudo tee $SYN_RES  &> /dev/null
        echo "nameserver 127.0.0.1"| sudo tee -a $SYN_RES &> /dev/null
        for VAR in ${SYN_DNS_CN[*]}; do
            echo "nameserver $VAR" | sudo tee -a $SYN_RES &> /dev/null
        done
    fi
