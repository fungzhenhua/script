...
SYN_AUTO_PATH="$HOME/.config/autostart"
SYN_AUTO="$SYN_AUTO_PATH/${SYN_NAME}.desktop"
...
SYN_SET_COF(){
if [ ! -e $SYN_AUTO ]; then
    if [ ! -d $SYN_AUTO_PATH ]; then
        mkdir -p $SYN_AUTO_PATH
    fi
    SYN_KEY_GET
    touch $SYN_AUTO
cat > $SYN_AUTO <<EOF
...
echo $SYN_KEY_X | sudo -S sh -c "cat > $SYN_RES" <<EOC
# Generated by syndns
