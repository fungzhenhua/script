...
DY_GET_INF(){
    if [ ! -e $DY_KEY_CFG ]; then
        ...
        echo "博客的模式，本地：0 , 远程：1" >> $DY_KEY_CFG
        ...
        if [[ ${DY_MODEL} == 0 ]]; then
            ...
            DY_PATH="$DY_SOURCE/.DY_LOCAL"
        else
            DY_PCLONESITE=${DY_INFO[3]}
            DY_PATH=$DY_SOURCE/${DY_INFO[4]}
        fi
        DY_URL="https://${DY_INFO[4]}"
        DY_TOKEN=${DY_INFO[5]}
